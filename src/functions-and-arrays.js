...
var resultadoSuma = sumNumbers(numbers);
console.log(`Este es el resultado del ejercicio 3: ${resultadoSuma}`);
...
const numbersAvg = [2, 6, 9, 10, 7, 4, 1];
const resultadoSuma2 = sumNumbers(numbersAvg);
...
function averageNumbers() {
  ...
  console.log(media);
  console.log(
    `hay ${items} valores que suman ${resultadoSuma2} y la media es ${media}`
  );
}
averageNumbers();
...
function averageWordLength(words) {
  ...
  for (let index = 0; index < words.length; index++) {
    suma += words[index].length;    
  }
  return Math.round(suma / words.length);
}
...
function howManyTimes(wordsCount,wordToCount) {
  ...
  for (let index = 0; index < wordsCount.length; index++) {
    if (wordsCount[index] === wordToCount) {
      count++;
    }
  }
  return count;
}
howManyTimes(wordsCount, wordToCount);
// Iteration #8: Bonus
const matrix = [
